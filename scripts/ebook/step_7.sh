...
ebook-convert $source_file $target_file --language en-US --no-default-epub-cover --cover "tmp/title.jpg" --book-producer "Torben Menke" --level1-toc "//h:h1" --level2-toc "//h:h2" --level3-toc "//h:h3" --filter-css "background-color"
#  --filter-css "background-color" is needed to remove the ugly gray background color calibre adds by default.
